...
class TritonPythonModel:
    ...
    def execute(self, requests):
        ...
        for request in requests:
            ...
            images = self.preprocess_input_image(images)
            
            detected_objects = self.postprocess(num_dets, det_boxes, det_scores, det_classes, images.shape[0], images.shape[1], [images.shape[0], images.shape[1]])
            
            crop_img = self.get_text_regions(images, detected_objects)
            
            out_tensor_0 = pb_utils.Tensor("OUTPUT_0",
                                           crop_img.astype(output0_dtype))
